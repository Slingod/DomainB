...
// Charger (ou créer) le schéma de base
const schema = fs.readFileSync(path.join(__dirname, 'schema.sql'), 'utf-8');
db.exec(schema);

// ---- Auto-migrations sûres ----
...
function ensureSortOrder() {
  if (hasColumn('products', 'sort_order')) return; // déjà OK

  const migrate = db.transaction(() => {
    // 1) Ajouter la colonne
    db.exec(`ALTER TABLE products ADD COLUMN sort_order INTEGER;`);

    // 2) Remplir une valeur cohérente :
    //    - si created_at est présent → utiliser strftime('%s',created_at)
    //    - sinon fallback sur id
    db.exec(`
      UPDATE products
